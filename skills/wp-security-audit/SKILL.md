...
description: Comprehensive WordPress plugin/theme security and code quality audit following WPCS and Plugin Check standards. Scans PHP files for SQL Injection, XSS, CSRF, Access Control, File Security, Forbidden Functions, Safe Redirect, Input Validation, and Code Quality issues. Use when reviewing security before releasing a plugin/theme, checking vulnerabilities, preparing for WordPress.org submission, or running a code quality audit.
...
Use the Agent tool with `subagent_type: general-purpose` (this plugin ships no
dedicated reviewer agent -- do not reference one). Pass the **complete** audit
rules from [references/audit-rules.md](references/audit-rules.md) as the prompt.

For a large codebase, split the work across parallel agents by directory
(`src/`, `templates/`, `includes/`, `admin/`) and merge the reports.
